...
export async function POST(req: NextRequest) {
  try {
    ...
    const { name, email, phone, company, service, message, referringPage } = body;
    ...
    if (resendKey) {
      try {
        ...
        await resend.emails.send({
          ...
          subject: `New Contact Form Submission: ${name} — ${company || 'No Company'}`,
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; background: #f9f9f9; border-radius: 8px;">
              ...
              <div style="background: #ffffff; padding: 24px; border-radius: 0 0 8px 8px; border: 1px solid #e0e0e0; border-top: none;">
                <table style="width: 100%; border-collapse: collapse;">
                  ...
                  <tr><td style="padding: 8px 0; color: #666; font-size: 13px;">Service Interest</td><td style="padding: 8px 0; font-size: 14px;">${service || '—'}</td></tr>
                ...
                <div style="margin-top: 20px; padding-top: 16px; border-top: 1px solid #e0e0e0;">
                  <a href="mailto:${email}?subject=Re: Your Centience Inquiry" style="display: inline-block; background: #e8a820; color: #0f1f3d; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 14px;">Reply to ${name.split(' ')[0]}</a>
                </div>
              </div>
