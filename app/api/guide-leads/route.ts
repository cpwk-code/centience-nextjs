...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1. Save to Supabase (best-effort — don't fail the request if DB is down)
    ...
    try {
      ...
      await addNoteToContact(
        contactId,
        `Downloaded guide: ${guideTitle || guideSlug || 'Governance guide'}${isReturning ? ' (returning visitor)' : ''}.${sourceUrl ? ` Source: ${sourceUrl}` : ''}`
      );
    } catch (hsErr) {
    ...
    if (resendKey) {
      try {
        ...
        await resend.emails.send({
          from: 'Centience Leads <notifications@website.cpwk.com>',
          to: 'hello@centience.ai',
          subject: `New Guide Download: ${subjectSafe(guideTitle, 80) || subjectSafe(guideSlug, 80) || 'Governance Guide'} — ${subjectSafe(fullName, 80)}`,
          html: `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 24px; background: #f9f9f9; border-radius: 8px;">
              <div style="background: #0f1f3d; padding: 20px 24px; border-radius: 8px 8px 0 0;">
                <h1 style="color: #e8a820; margin: 0; font-size: 20px;">New Guide Download Lead</h1>
                <p style="color: #ffffff; margin: 4px 0 0; font-size: 14px;">${safe(guideTitle, 160) || safe(guideSlug, 160) || 'Governance Guide'}</p>
              </div>
              <div style="background: #ffffff; padding: 24px; border-radius: 0 0 8px 8px; border: 1px solid #e0e0e0; border-top: none;">
                <table style="width: 100%; border-collapse: collapse;">
                  ...
                  <tr><td style="padding: 8px 0; color: #666; font-size: 13px;">Downloaded</td><td style="padding: 8px 0; font-size: 14px;">${safe(guideTitle, 160) || safe(guideSlug, 160) || '—'}</td></tr>
                ...
                <div style="margin-top: 20px; padding-top: 16px; border-top: 1px solid #e0e0e0;">
                  <a href="mailto:${encodeURIComponent(email)}?subject=Re: ${encodeURIComponent(capped(guideTitle, 160) || 'Governance Guide')} Download" style="display: inline-block; background: #e8a820; color: #0f1f3d; padding: 10px 20px; border-radius: 6px; text-decoration: none; font-weight: bold; font-size: 14px;">Reply to ${safe(firstName, 60)}</a>
                </div>
              </div>
    ...
    console.error('Guide leads API error:', err);
